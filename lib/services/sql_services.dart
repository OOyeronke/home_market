...
class SQLService {
  ...
  Future saveRecord(MarketItemModel data) async {
    await this.db?.transaction((txn) async {
      var qry =
          'INSERT INTO shopping(name, subtitle, price, image,rating,fav) VALUES("${data.name}","${data.subtitle}", ${data.price}, "${data.image}",${data.rating},${data.fav ? 1 : 0})';
  ...
  Future addToCart(MarketItemModel data) async {
    await this.db?.transaction((txn) async {
      var qry =
          'INSERT INTO cart_list(shop_id, name, subtitle, price, image,rating,fav) VALUES(${data.id}, "${data.name}", "${data.subtitle}", ${data.price}, "${data.image}",${data.rating},${data.fav ? 1 : 0})';
      int id1 = await txn.rawInsert(qry);
      return id1;
    });
  }
  ...
  Future removeFromFav(int shopId) async {
    var qry = "DELETE FROM cart_list where shop_id = ${shopId}";
    return await this.db?.rawDelete(qry);
  }
}
